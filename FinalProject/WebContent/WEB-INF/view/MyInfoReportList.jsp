...
<body>
	...
	<div class="container">
		...
		<div class="page-header">
			<h1 class="page-heading">나의 신고리스트</h1>
		</div>
		<br>
		<br>
							...
							<c:forEach var="myinfoAllList" items="${myinfoAllList }">
								<tr>
									...
									<td style="text-align: center">
										<button type="button" class="btn btn-light" style="height: 24pt; font-size: 13px;" onclick="location.href='readcheck.action?checkNo=${myinfoAllList.check_no}'">이동</button>
									</td>
									...
									<!-- 취소 버튼은 미해결일 때만 클릭 가능 -->
									<c:choose>
										<c:when test="${myinfoAllList.statusname eq '미해결'}">
											<td><button type="button" class="btn btn-secondary" id="cancleBtn" value="${myinfoAllList.rpcheck_no }" style="height: 24pt; font-size: 13px;">취소</button></td>
										</c:when>
										<c:otherwise>
											<td><button type="button" class="btn btn-secondary" id="cancleBtn" value="${myinfoAllList.rpcheck_no }" style="height: 24pt; font-size: 13px;" disabled="disabled">취소</button></td>
										</c:otherwise>
									</c:choose>
